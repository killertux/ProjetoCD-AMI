...
void AMI_Client::sendMessage(){
	...
	std::string cmd = (data.find(" ")!=data.npos)?data.substr(0,data.find(" ")):data;
	if(cmd == "send"){
		...
		std::string message = data.substr(data.find(" "),data.size());
		int foo = message.find(" ");
		std::string person = message.substr(0,foo);
		message = message.substr(foo,message.size());
